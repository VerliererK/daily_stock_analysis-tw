...
from src.report_language import normalize_report_language, get_active_report_language_variant
...
def _build_language_section(report_language: str, *, chat_mode: bool = False) -> str:
    ...
    zh_tw = get_active_report_language_variant() == "zh-tw"
    if chat_mode:
        if normalized == "en":
            return """
## Output Language
...
- 預設使用**台灣繁體中文**回答，採用台灣金融慣用語（本益比、殖利率、法人買賣超、量能）。
- 若輸出 JSON，鍵名保持不變，所有面向使用者的文字使用台灣繁體中文。
"""
        return """
## 输出语言
...
- 所有面向使用者的人類可讀文字必須使用**台灣繁體中文**，採用台灣金融慣用語（本益比、殖利率、法人買賣超、量能、漲跌停），不要使用簡體字。
"""
