...
- `SCHEDULE_MAX_CONSECUTIVE_FAILURES`：已运行的 schedule 模式会在下一次任务前读取新的连续失败门槛；达到门槛并首次进入暂停状态时，会通过已配置通知渠道发送一次操作告警，使用 `--no-notify` 启动时不会发送；已暂停的 `scheduler_failure_guard.json` 不会因保存该字段自动解除，需要修复问题后手动删除。
