...
- symbol 目标使用 `get_market_for_stock(normalize_stock_code(effective_target))` 构造触发时 phase；`target_scope=market` 直接用 `normalize_market_region(target)`，不会把 `cn|hk|us|tw` 当作股票代码推断；账户级无法唯一定位市场时允许 summary 落为 `unknown`。
...
| --- | --- | --- | --- | --- |
| `market` | `cn` / `hk` / `us` / `tw` | `market_light_status` | `statuses=["red","yellow"]`，只允许 `red/yellow`，默认 `["red","yellow"]` | 当前 `MarketLightSnapshot.status` 命中列表时触发 |
| `market` | `cn` / `hk` / `us` / `tw` | `market_light_score_drop` | `min_drop > 0` | `prev.score - current.score >= min_drop`，且 `prev.trade_date < current.trade_date` |
